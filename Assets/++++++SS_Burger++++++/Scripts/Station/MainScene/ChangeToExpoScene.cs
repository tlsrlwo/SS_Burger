...
public sealed class ChangeToExpoScene : StationBase
{
    [Header("엑스포")]
    ...
    public override void OnPlayerArrive(GameObject player, ArrivalDetector.ArrivalEvents events)
    {
        ...
        Debug.Log($"[ExpoStation] ' {playerName} ' 이(가) ' {stationName} '(StationID = {StationID}' 에 도착함." +
    ...
    public override void OnPlayerLeave(GameObject player)
    {
        ...
        Debug.Log($"[ExpoStation] Player '{playerName}' 가 Expo '{stationName}' (StationID='{StationID}') 에서 이탈함. ");
