...
public sealed class ChangeToBunScene : StationBase
{
    [Header("번")]
    [SerializeField] private string stationName = "Bun";        // 디버그 식별용
    ...
    public override void OnPlayerArrive(GameObject player, ArrivalDetector.ArrivalEvents events)
    {
        ...
        Debug.Log($"[BunStation] ' {playerName} ' 이(가) ' {stationName} '(StationID = {StationID}' 에 도착함." +
    ...
    public override void OnPlayerLeave(GameObject player)
    {
        ...
        Debug.Log($"[BunStation] Player '{playerName}' 가 Bun '{stationName}' (StationID='{StationID}') 에서 이탈함. ");
