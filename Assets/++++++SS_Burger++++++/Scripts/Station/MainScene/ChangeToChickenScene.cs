...
public sealed class ChangeToChickenScene : StationBase
{
    [Header("치킨")]
    ...
    public override void OnPlayerArrive(GameObject player, ArrivalDetector.ArrivalEvents events)
    {
        ...
        Debug.Log($"[ChickenStation] ' {playerName} ' 이(가) ' {stationName} '(StationID = {StationID}' 에 도착함." +
    ...
    public override void OnPlayerLeave(GameObject player)
    {
        ...
        Debug.Log($"[ChickenStation] Player '{playerName}' 가 Chicken '{stationName}' (StationID='{StationID}') 에서 이탈함. ");
